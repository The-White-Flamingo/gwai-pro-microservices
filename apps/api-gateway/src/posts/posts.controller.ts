...
import { postMediaUploadOptions, toPostMediaPath } from './post-media.util';
...
export class PostsController {
  ...
  @ApiOperation({
    ...
    description:
      'Only musicians and studios can create feed posts. A post can be caption-only, media-only, or media plus caption. Only images for now',
  })
  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        caption: {
          type: 'string',
          example: 'New studio setup ready for tonight.',
        },
        media: {
          type: 'string',
          format: 'binary',
  ...
  create(
    @Body() body: Omit<CreatePostDto, 'userId' | 'userRole' | 'mediaUrl'>,
    @UploadedFile() file: Express.Multer.File,
    @ActiveUser() user: ActiveUserData,
  ) {
    return this.postsService.create({
      ...body,
      mediaUrl: toPostMediaPath(file),
      userId: user.sub,
      userRole: user.role,
    });
  }
  ...
  update(
    @Param('id') id: string,
    @Body() body: Partial<Omit<UpdatePostDto, 'id' | 'userId' | 'userRole' | 'mediaUrl'>>,
    @UploadedFile() file: Express.Multer.File,
    @ActiveUser() user: ActiveUserData,
  ) {
